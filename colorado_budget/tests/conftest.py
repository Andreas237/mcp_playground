...
@pytest.fixture(scope="session")
def agriculture_server(api_keys):
    """Start the colorado-agriculture MCP server and yield; stop on teardown."""
    script = SRC / "servers" / "agriculture.py"
    proc = subprocess.Popen(
        [sys.executable, str(script)],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    )
    _wait_for_port(8010, "colorado-agriculture", timeout=20)
    yield proc
    proc.terminate()
    proc.wait()
...
@pytest.fixture(scope="session")
def cdot_server(api_keys):
    """Start the colorado-cdot MCP server and yield; stop on teardown."""
    script = SRC / "servers" / "cdot.py"
    proc = subprocess.Popen(
        [sys.executable, str(script)],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    )
    _wait_for_port(8011, "colorado-cdot", timeout=20)
    yield proc
    proc.terminate()
    proc.wait()
